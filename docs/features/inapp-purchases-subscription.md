...
:::info No premium features?
`PREMIUM_FEATURES_ENABLED` in `root/AppConfiguration.kt` controls whether the app has any premium (paid/gated) features — not whether it's free to download. Set it `false` when there are **no premium features**: everything is unlocked and free — no paywall, no subscriptions/upgrade UI, billing never initializes, credits off (generation free). Leave it `true` (the default) for everything below — the app can still be free to download while gating some features. Ads are independent (see [AdMob Ads](./admob-ads.md)).
:::
...
The custom paywall lives at `shared/src/commonMain/kotlin/com/kotlinfoundation/koko/presentation/screens/paywall/` and is split into three layers so that screens stay display-only and the formatting logic is unit-testable in isolation.
