...
`PREMIUM_FEATURES_ENABLED` in `root/AppConfiguration.kt` controls whether the app has any premium (paid/gated) features — not whether it's free to download. It is **`false` by default**: everything is unlocked and free — no paywall, no subscriptions/upgrade UI, billing never initializes, credits off (generation free). **Set it to `true` to use anything below** — the app can still be free to download while gating some features. Ads are independent (see [AdMob Ads](./admob-ads.md)).
:::
