...
Every PR runs format, lint, unit tests, Compose UI tests, screenshot regression tests, and an Android debug build via [`.github/workflows/pr_checks.yml`](https://github.com/KotlinFoundation/kmp-contest-starter-kit/blob/main/.github/workflows/pr_checks.yml). Run the same gates locally before pushing.
...
Powered by [Roborazzi](https://github.com/takahirom/roborazzi) + [ComposablePreviewScanner](https://github.com/sergio-sastre/ComposablePreviewScanner). Every `@Preview` under `com.kotlinfoundation.koko.*` is automatically discovered, rendered via Robolectric, and snapshotted. **You don't write a test class** — just add a `@Preview` and re-run.
