...
SSH:
```bash
git clone git@github.com:KotlinFoundation/kmp-contest-starter-kit.git
```
or HTTPS:
```bash
git clone https://github.com/KotlinFoundation/kmp-contest-starter-kit.git
```
...
changes. See [iOS Production](../production/iOS.md#swiftpm-dependencies--the-linkage-package) for details.
:::
...
Your apps now run, with authentication, notifications, and in-app purchases already
...
- **`firebase deploy --only functions` fails with a `403` / Secret Manager error** — enable the Secret Manager API for the project, then re-deploy. See [Firebase Integration](../features/firebase-integration.md).
