...
Update the following fields in `shared/src/commonMain/.../root/AppConfiguration.kt`:

- [ ] `PREMIUM_FEATURES_ENABLED` — does the app have any premium (paid/gated) features? Leave `true` if it does (subscriptions/paywall/credits — the app can still be free to download). Set `false` when there are **no premium features** — everything is unlocked and free: no paywall, no subscriptions/upgrade UI, billing never initializes, credits off (generation free). When `false`, skip the Subscription Provider steps below.
...
If using social login (`AUTH_SOCIAL_LOGIN_ENABLED = true` in AppConfiguration):
...
- [ ] **AI backend uses the Firebase proxy, not on-device keys.** If you used the prototyping
  `OPENAI_API_KEY` / `REPLICATE_API_KEY` (direct mode), **remove them** and set `CLOUD_FUNCTIONS_URL` so
  the app calls the proxy — direct keys compile into the app binary. Make sure `USE_AI_PROXY_SERVER`
  isn't forcing direct mode. See [AI Integration](../features/ai-integration.md).

> **Readiness check.** Run `./scripts/check_env.sh --phase publishing` from `MobileApp/` — it flags any
> unset privacy/terms URL, boilerplate contact email, on-device AI keys, or unset (mock) subscription keys.

---

## 9. Subscription Provider

- [ ] **Choose your provider** in `gradle.properties` (this is the only switch — `AppConfiguration` resolves the factory automatically, no code change needed):
  ```properties
