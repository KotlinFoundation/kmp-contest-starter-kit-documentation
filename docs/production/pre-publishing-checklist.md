...
- [ ] `PREMIUM_FEATURES_ENABLED` — does the app have any premium (paid/gated) features? Defaults to `false`: everything is unlocked and free — no paywall, no subscriptions/upgrade UI, billing never initializes, credits off (generation free); skip the Subscription Provider steps below. Set it `true` if the app gates features behind subscriptions/paywall/credits (it can still be free to download).
